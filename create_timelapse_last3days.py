...
def create_timelapse_last3days():
    """
    Creates a high-quality web-compatible MP4 timelapse video from images in the 'photos' directory from the last 3 days only.
    """
    photos_dir = 'photos'
    output_dir = '_site'
    output_video = os.path.join(output_dir, 'timelapse_last3days.mp4')
    ...
    # Calculate the date 3 days ago (date only, not datetime)
    ...
    # Only include files matching the expected filename pattern and within last 3 days
    ...
    for f in image_files:
        ...
        try:
            date_obj = datetime.datetime.strptime(folder, "%Y-%m-%d").date()
            # Only include images from the last 3 days
    ...
    if not image_files:
        print("No images found from the last 3 days to create a timelapse video.")
        return

    print(f"Found {len(image_files)} images from the last 3 days. Creating high-quality MP4 timelapse video with timestamps...")
    ...
    try:
        import subprocess
        temp_video = os.path.join(output_dir, 'timelapse_last3days_temp.mp4')
        ...
        print(f"Final high-quality timelapse video (last 3 days) saved to {output_video}")
        ...
        if os.path.exists(temp_video):
            ...
            print(f"Basic timelapse video (last 3 days) saved to {output_video}")

if __name__ == "__main__":
    create_timelapse_last3days()